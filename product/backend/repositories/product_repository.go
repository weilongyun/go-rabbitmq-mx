...
func (p *ProductRepositoryManager) Insert(product *datamodels.Product) (id int64, err error) {
	...
	//预编译采用占位符，防止sql注入
	sql := "insert " + p.table + " set product_id=?,product_name=?,product_num=?,product_image=?,product_price=?"
...
//通过商品id删除
func (p *ProductRepositoryManager) Delete(id string) (isSuccess bool, err error) {
	if err := p.Conn(); err != nil {
		log.Fatalln("mysql ProductRepositoryManager Delete error", err)
		return false, err
	}
	sql := "delete from " + p.table + " where id=?"
	log.Println("mysql Delete  sql", sql)
	stmt, err := p.mysqlConn.Prepare(sql)
	if err != nil {
...
func (p *ProductRepositoryManager) Update(product *datamodels.Product) (err error) {
	...
	sql := "update " + p.table + " set product_id=?,product_name=?,product_num=?,product_image=?,product_price=? where id=?"
	log.Println("mysql Update  sql", sql)
	stmt, err := p.mysqlConn.Prepare(sql)
	if err != nil {
		log.Fatalln("mysql ProductRepositoryManager Update Prepare error", err)
		return
	}
	_, err = stmt.Exec(product.ProductID, product.ProductName, product.ProductNum, product.ProductImage, product.ProductPrice, product.ID)
	if err != nil {
